...
struct nkpoint : public Worker {
    ...
    nkpoint(
        ...
        :  DR(D), maskR(mask), trapsR(traps), dist2R(dist2), detectR(detect), TskR(Tsk), 
            markoccR(markocc), fn(fn), gsbR(gsb), miscparmR(miscparm), w2(w2), binomNR(binomN), 
            nk(nk) {
    ...
    
{
    ...
    NumericVector nk(ntraps);
    
    nkpoint npoint (D, mask, traps, dist2, detect, Tsk, markocc, fn, gsb, miscparm, w2, binomN, nk);
    
    if (ncores>1) {
        parallelFor(0, ntraps, npoint, grain, ncores);
    }
    else {
        npoint.operator()(0, ntraps);    // for debugging avoid multithreading to allow R calls
    }

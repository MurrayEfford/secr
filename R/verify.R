...
## 2026-06-29 reformat code
## 2026-06-29 report telemetry data errors
############################################################################################
...
verify.capthist <- function (object, report = 2, tol = 0.01, ...) {
    ...
    if (inherits(object, 'list')) {
        ...
        if (length(object)==0) {
            detectionsOK <- unmarked   # obsolete 2026-06-29?
        }
        else  {
            ...
            ## single, multi, capped, proximity, polygonX, transectX, 
            ## signal and signalnoise must be binary
        ...
        if (telem) {
            ...
            if (any(telemdet.byoccasion == 0)) {
                telemOK <- FALSE
                telemetryerror <- "telemetry occasions with no detections"
            }
            if (length(animaldet) != sum(telemetrd)) {
                telemOK <- FALSE
                telemetryerror <- "mismatch of animals"
            }
            else if (any(sort(names(animaldet)) != sort(names(telemdet.byanimal[telemetrd])))) {
                telemOK <- FALSE
                telemetryerror <- "mismatch of names"
            }
                ...
                telemOK <- FALSE
                telemetryerror <- "non-zero detection histories for independent telemetry"
            }
        }
        
        ##############################
        ## nontargetdimOK, nontargetOK
